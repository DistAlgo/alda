...
"""
			...
			src = func_template.format(KW_QUERY=KW_QUERY, 
									   RuleTarget=tarfile, 
									   KWARGS=', '.join(['%s=%s'%(a.name, a.name) for a in node.args.args]),
									   ReturnVars=', '.join(["'%s'"%v for v in cspAST.objective.variables]),
									   OBJNAME=None if not cspAST.objective.objective or not isinstance(cspAST.objective.objective.obj, str) else "'%s'" %cspAST.objective.objective.obj)
	...
	def visit_Program(self, node):
		...
		imptquery.module = 'da.constraint.query'
