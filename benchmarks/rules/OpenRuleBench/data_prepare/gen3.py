...
import random, sys, copy, time, pickle, re
...
# ------------ systemList is a list of systems to generate data
	def SavePrologGraph(self, systemList, _fileName = ""):
		if _fileName == "":
			_fileName = self.name
		for sys in systemList:
			if sys in ['xsb', 'yap', 'dlv', 'iris']:
				file1 = _fileName + '_' + sys
				file2 = _fileName + '_' + sys
				if sys == 'xsb':
					file1 = file1 + '_cyc.P'
					file2 = file2 + '_nocyc.P'
				elif sys == 'yap':
					file1 = file1 + '_cyc.pl'
					file2 = file2 + '_nocyc.pl'
				else:
					file1 = file1 + '_cyc.' + sys
					file2 = file2 + '_nocyc.' + sys
				fout1 = open(file1, 'w')
				fout2 = open(file2, 'w')
				for _relation in self.lRelations:
					for _tuple in _relation.rel_content:
						# with cycles allowed
						fout1.write(_relation.name + "(")
						fout1.write(str(_tuple[0]) + "," + str(_tuple[1]) + ").\n")
						# without cycles allowed
						fout2.write(_relation.name + "(")
						if _tuple[0] > _tuple[1]:
							fout2.write(str(_tuple[1]) + "," + str(_tuple[0]) + ").\n")
						else:
							fout2.write(str(_tuple[0]) + "," + str(_tuple[1]) + ").\n")
				fout1.close()
				fout2.close()
				print('    generated: ' + file1)
				print('    generated: ' + file2)
			elif sys in ['jena', 'drools', 'owlim']:
				file1 = _fileName + '_' + sys
				file2 = _fileName + '_' + sys
				file1 = file1 + '_cyc.' + sys
				file2 = file2 + '_nocyc.' + sys
				...
				for _relation in self.lRelations:
					for _tuple in _relation.rel_content:
						# with cycles allowed
						fout1.write(_relation.name + "\n")
						fout1.write(str(_tuple[0]) + "\n" + str(_tuple[1]) + "\n")
						# without cycles allowed
						fout2.write(_relation.name + "\n")
						if _tuple[0] > _tuple[1]:
							fout2.write(str(_tuple[1]) + "\n" + str(_tuple[0]) + "\n")
						else:
							fout2.write(str(_tuple[0]) + "\n" + str(_tuple[1]) + "\n")
				fout1.close()
				fout2.close()
				print('    generated: ' + file1)
				print('    generated: ' + file2)
			elif sys == 'jess':
				file1 = _fileName + '_' + sys + '_cyc.jess'
				file2 = _fileName + '_' + sys + '_nocyc.jess'
				fout1 = open(file1, 'w')
				fout2 = open(file2, 'w')
				for _relation in self.lRelations:
					for _tuple in _relation.rel_content:
						# with cycles allowed
						fout1.write("(" + _relation.name + " ")
						fout1.write("(first "  + str(_tuple[0]) + ") ")
						fout1.write("(second " + str(_tuple[1]) + "))\n")
						# without cycles allowed
						fout2.write("(" + _relation.name + " ")
						if _tuple[0] > _tuple[1]:
							fout2.write("(first "  + str(_tuple[1]) + ") ")
							fout2.write("(second " + str(_tuple[0]) + "))\n")
						else:
							fout2.write("(first "  + str(_tuple[0]) + ") ")
							fout2.write("(second " + str(_tuple[1]) + "))\n")
				fout1.close()
				fout2.close()
				print('    generated: ' + file1)
				print('    generated: ' + file2)
			elif sys in ['ontoBroker']:
				file1 = _fileName + '_' + sys
				file2 = _fileName + '_' + sys
				file1 = file1 + '_cyc.' + sys
				file2 = file2 + '_nocyc.' + sys
				fout1 = open(file1, 'w')
				fout2 = open(file2, 'w')
				if re.compile('tc').search(_fileName):
					fout1.write("tc(?X,?Y) :- par(?X,?Z) AND tc(?Z,?Y).\ntc(?X,?Y) :- par(?X,?Y).\n")
					fout2.write("tc(?X,?Y) :- par(?X,?Z) AND tc(?Z,?Y).\ntc(?X,?Y) :- par(?X,?Y).\n")
				else:
					fout1.write("sg(?X,?X) :- par(?Y,?X).\nsg(?X,?Y) :- par(?X,?Z) AND sg(?Z,?Z1) AND par(?Y,?Z1).\nsg(?X,?X) :- sib(?X,?Y).\nsg(?X,?X) :- par(?X,?Y).\nsg(?X,?X) :- sib(?Y,?X).\nsg(?X,?Y) :- sib(?X,?Y).\n")
					fout2.write("sg(?X,?X) :- par(?Y,?X).\nsg(?X,?Y) :- par(?X,?Z) AND sg(?Z,?Z1) AND par(?Y,?Z1).\nsg(?X,?X) :- sib(?X,?Y).\nsg(?X,?X) :- par(?X,?Y).\nsg(?X,?X) :- sib(?Y,?X).\nsg(?X,?Y) :- sib(?X,?Y).\n")
				for _relation in self.lRelations:
					for _tuple in _relation.rel_content:
						# with cycles allowed
						fout1.write(_relation.name + "(")
				...
				for _relation in self.lRelations:
					for _tuple in _relation.rel_content:
						# with cycles allowed
						fout1.write("(CYC-ASSERT-WFF '(#$"+_relation.name + "-predicate #$node-")
						fout1.write(str(_tuple[0]) + " #$node-" + str(_tuple[1]) + ") #$BaseKB)\n")
						# without cycles allowed
						fout2.write("(CYC-ASSERT-WFF '(#$"+_relation.name + "-predicate #$node-")
						if _tuple[0] > _tuple[1]:
							fout2.write(str(_tuple[1]) + " #$node-" + str(_tuple[0]) + ") #$BaseKB)\n")
						else:
							fout2.write(str(_tuple[0]) + " #$node-" + str(_tuple[1]) + ") #$BaseKB)\n")
				fout1.close()
				fout2.close()
				print('    generated: ' + file1)
				print('    generated: ' + file2)
...
# ------------ preprocess domains for generation
	...
	# shorten domains according to 3rd constraint by picking the specified number of random integers from domain
		dPrimeDoms = {}
                ## annie: next line was random.jumpahead(1)
		random.seed(random.randrange(0, 0x5EEDF00D))
		for i in self.RANGE_ATTR_NUM:
			print('====21:', dPrimeDoms, self.primed_attr_sizes[i])
		...
		# create a dictionary linking attribute sizes and domains
			while self.primed_attr_sizes[i] in dPrimeDoms:
				self.primed_attr_sizes[i] += 1 # avoid similar keys, the new size will never exceed the primed size
			dPrimeDoms[self.primed_attr_sizes[i]] = list(self.doms_content[i])
