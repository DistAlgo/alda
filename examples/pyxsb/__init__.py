...
def pyxsb_start_session(xsb_arch_dir_arg=None, other_args=[]):
    ...
    if not xsb_arch_dir_arg:
        ...
        if our_platform == 'Windows':
            ...
            xsb_arch_dir = os.environ["HOME"] + '/XSB/XSB/config/i386-apple-darwin18.0.0'
...
def check_session_parameter(param, additional_file, param_type):
    ...
    if (not os.path.isdir(param)) or \
       (not os.path.exists(param + additional_file)):
        # print('param='+param)
        # print(os.path.isdir(param))
        # print('param+additional_file='+param+additional_file)
        # print(os.path.exists(param + additional_file))
