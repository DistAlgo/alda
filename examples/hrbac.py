...
class CoreRBAC(da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, **rest_1579):
        super().setup(_users=_users, _roles=_roles, _ur=_ur, **rest_1579)
    ...
    def AssignedUsers(self, role):
        ...
        return {u for (u, _BoundPattern384_) in self._state.UR if (_BoundPattern384_ == role)}

    def AssignedRoles(self, user):
        'the set of roles assigned to user in UR'
        return {r for (_BoundPattern397_, r) in self._state.UR if (_BoundPattern397_ == user)}

    def UserPermissions(self, user):
        'the set of permissions assigned to the roles assigned to user'
        return {p for (_BoundPattern412_, r) in self._state.UR if (_BoundPattern412_ == user) for (p, _FreePattern421_) in self._state.PR if (_FreePattern421_ == r)}
...
class HierarchicalRBAC(CoreRBAC, da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, _rh, _workload, _outfile, **rest_1579):
        super().setup(_users=_users, _roles=_roles, _ur=_ur, _rh=_rh, _workload=_workload, _outfile=_outfile, **rest_1579)
        self._state._users = _users
        self._state._roles = _roles
        self._state._ur = _ur
        self._state._rh = _rh
        ...
        self._state.tmpout = 'tmpout'
        self._state.tmpfout = open(self._state.tmpout, 'a')
    ...
    def AuthorizedUsers(self, _role):
        ...
        return {u for (u, asc) in self._state.UR for (_FreePattern527_, _BoundPattern529_) in transRH if (_FreePattern527_ == asc) if (_BoundPattern529_ == role)}
...
class HRBAC_set(HierarchicalRBAC, da.DistProcess):
    ...
    def trans(self, E):
        ...
        def ExistentialOpExpr_811():
            nonlocal x, y, z
            for (x, y) in T:
                for (_FreePattern822_, z) in E:
                    if (_FreePattern822_ == y):
                        if (not ((x, z) in T)):
                            return True
            return False
        while ExistentialOpExpr_811():
            T.add((x, z))
        return (T | {(r, r) for r in self._state.ROLES})
    ...
    def AuthorizedUsers(self, role):
        'the set of users of role or ascendant roles of role'
        transRH = self.trans(self._state.RH)
        return {u for (u, asc) in self._state.UR for (_FreePattern877_, _BoundPattern878_) in transRH if (_FreePattern877_ == asc) if (_BoundPattern878_ == role)}
    ...
    def AuthorizedRoles(self, user):
        ...
        return {r for (_BoundPattern899_, asc) in self._state.UR if (_BoundPattern899_ == user) for (_FreePattern906_, r) in self._state.transRH if (_FreePattern906_ == asc)}
...
class HRBAC_transRH_rules(HRBAC_set_maint, da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, _rh, _workload, _outfile):
        ...
        self._rules_object = {'HRBAC_transRH_rules': {'LhsVars': {('transRH', 2)}, 'RhsVars': {}, 'Unbounded': {'roles', 'rh'}}}
...
class HRBAC_transRH_rs(HierarchicalRBAC, da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, _rh, _workload, _outfile, **rest_1579):
        ...
        self._rules_object = {'HRBAC_transRH_rs': {'LhsVars': {('transRH', 2)}, 'RhsVars': {'ROLES', 'RH'}, 'Unbounded': {}}}
        self._state.transRH = self.infer(rule='HRBAC_transRH_rs', queries=['transRH(_,_)'])

    def AuthorizedUsers(self, role):
        return {u for (u, asc) in self._state.UR for (_FreePattern1093_, _BoundPattern1094_) in self._state.transRH if (_FreePattern1093_ == asc) if (_BoundPattern1094_ == role)}
...
class HRBAC_trans_rules(HRBAC_py, da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, _rh, _workload, _outfile):
        super().setup(_users, _roles, _ur, _rh, _workload, _outfile)
        self._rules_object = {'HRBAC_trans_rules': {'LhsVars': {}, 'RhsVars': {}, 'Unbounded': {'edge'}}}

    def trans(self, E):
        i = self.infer([('edge', E)], ['path(_,_)'], 'HRBAC_trans_rules')
...
class HRBAC_trans_rules_all(HRBAC_py, da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, _rh, _workload, _outfile):
        super().setup(_users, _roles, _ur, _rh, _workload, _outfile)
        self._rules_object = {'HRBAC_trans_rules_all': {'LhsVars': {}, 'RhsVars': {}, 'Unbounded': {'ur', 'edge'}}}

    def AuthorizedUsers(self, role):
        i = self.infer([('edge', self._state.RH), ('ur', self._state.UR)], ['authuser(_,{ROLE})'.format(ROLE=role)])
        return (set(i) | {(u,) for (u, _BoundPattern1165_) in self._state.UR if (_BoundPattern1165_ == role)})
...
class HRBAC_trans_with_role_rules(HRBAC_py, da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, _rh, _workload, _outfile):
        super().setup(_users, _roles, _ur, _rh, _workload, _outfile)
        self._rules_object = {'trans_with_role_rules': {'LhsVars': {}, 'RhsVars': {}, 'Unbounded': {'edge', 'role'}}}
...
class HRBAC_trans_with_role_rules_all(HRBAC_py, da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, _rh, _workload, _outfile):
        super().setup(_users, _roles, _ur, _rh, _workload, _outfile)
        self._rules_object = {'HRBAC_trans_with_role_rules_all': {'LhsVars': {}, 'RhsVars': {}, 'Unbounded': {'ur', 'edge', 'roles'}}}
...
class HRBAC_trans_with_ROLES_rules(HRBAC_set, da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, _rh, _workload, _outfile):
        super().setup(_users, _roles, _ur, _rh, _workload, _outfile)
        self._rules_object = {'trans_with_ROLES_rules': {'LhsVars': {}, 'RhsVars': {'ROLES'}, 'Unbounded': {'edge'}}}
...
class HRBAC_transRH_with_edge_rules(HRBAC_set, da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, _rh, _workload, _outfile):
        super().setup(_users, _roles, _ur, _rh, _workload, _outfile)
        self._rules_object = {'transRH_with_edge_rules': {'LhsVars': {('transRH', 2)}, 'RhsVars': {'ROLES'}, 'Unbounded': {'edge'}}}
...
class HRBAC_trans_with_RH_ROLES_rules(HRBAC_set, da.DistProcess):
    ...
    def setup(self, _users, _roles, _ur, _rh, _workload, _outfile):
        super().setup(_users, _roles, _ur, _rh, _workload, _outfile)
        self._rules_object = {'trans_with_RH_ROLES_rules': {'LhsVars': {}, 'RhsVars': {'ROLES', 'RH'}, 'Unbounded': {}}}
